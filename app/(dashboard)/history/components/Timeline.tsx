...
const JourneyTimeline = ({ journey }: { journey: JourneyItem[] }) => {

    // const parseJourney = (journey: string[]): JourneyItem[] => {
            ...
            {journey ? <ol className="relative border-s border-gray-200 dark:border-gray-500">
                {journey.length != 0 && journey.map((item, index) => (
                    <li key={index} className="mb-10 ms-6">
                        ...
                        <h3 className="flex items-center mb-1 pt-1 font-semibold text-gray-900 dark:text-white">{item.title}</h3>
